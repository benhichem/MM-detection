...
class MM {
    ...
    async _DownloadFiles() {
        ...
        this._DemoLinks.map(async (item) => {
            ...
            try {
                const ExecDownload = await execSync(`rclone copyurl ${Link} ./dist/Downloads/${SteamID}/${SteamID}.dem.gz`);
                console.log("Starting Extraction");
                //Extraction and Deleting the Old file
                const ExtractFile = await execSync(`gunzip ./dist/Downloads/${SteamID}/${SteamID}.dem.gz`);
    ...
    async _ProcessFile() {
        ...
        FilesNames.map(async (FileName) => {
            try {
                ...
                let Execution = await execSync(`python3 ./processor/test.py ${FileName}/`);
