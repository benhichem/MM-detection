...
class MM {
  ...
  private _DemoLinks: { SteamID: string; Link: string }[] | null;

  private _Payload: {
    /**
     *@disc the number of Links in the CSV File
     */
    _DemoLinksAvaliable: number;
    /**
     * @disc Files that are succsefully download and Extracted
     */

    _FileDownloadAndExtracted: number;

    /**
     * @disc Files That failed to be downloaded for reason like the link is Broken
     */

    _FileFailedToDownload: number;

    /**
     * @disc files Names and Paths that failed the Download Part
     */

    _LinksDownloadFail: { SteamID: string; Link: string }[];

    /**
     * @disc The Number of demos that are succsefully Processed
     */

    _DemosSuccefullyProccessed: number;

    /**
     * @disc The Number Of demos that failed to Process
     */

    _DemoosFailedProcessing: number;

    /**
     * @disc Paths To Demos that Failed To Process
     */

    _PathsToDemosThatFailedProcessing: string[];
  };
...
console.log(await new MM(argumnts[3])._exec());
